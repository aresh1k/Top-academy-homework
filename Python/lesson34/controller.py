...
class Controller:
    ...
    def check_user_answer(self, answer):
        if answer == "1":
            ...
            film_title = self.user_interface.get_user_film()
            try:
                producer = self.film_model.get_film_producer(film_title)
            except KeyError:
                self.user_interface.show_incorrect_error(film_title)
            else:
                self.user_interface.show_film_producer(producer)
        elif answer == '6':
            film_title = self.user_interface.get_user_film()
            try:
                duration = self.film_model.get_film_duration(film_title)
            except KeyError:
                self.user_interface.show_incorrect_error(film_title)
            else:
                self.user_interface.show_film_duration(duration)
        elif answer == "q":
